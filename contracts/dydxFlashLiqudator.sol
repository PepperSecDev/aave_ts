...
contract AaveLiquidate is ReentrancyGuard, Ownable, Structs {
  ...
  function liquidate(address _victim) external nonReentrant returns(uint profit) {
    ...
    profit = IERC20(USDC).balanceOf(address(this))
    IERC20(USDC).safeTransfer(msg.sender, profit);
    return profit
  }
